...
class ImageLoader:
    ...
    def __init__(self):
        ...
        objects_dir = path.join(img_dir, 'object_spsheet')
        obj_img_paths = {key: path.join(objects_dir, objects_data[key]) for key in objects_data}
        self.__init_spritesheet(obj_img_paths, self.spritesheets_data['objs'])

        #Initialize UI spritesheet
        ui_dir = path.join(img_dir, 'ui_spsheet')
        ui_img_paths = {key: path.join(ui_dir, ui_data[key]) for key in ui_data}
        self.__init_spritesheet(ui_img_paths, self.spritesheets_data['ui'])
    ...
    def get_image(self, fn):
        ...
        img = objs_dict['preloads'].get(fn, self.__create_surface(fn, objs_dict))
        ...
        # If didn't find, check UI spritesheet
            ...
            return ui_dict['preloads'].get(fn, self.__create_surface(fn, ui_dict))

    def __create_surface(self, filename, spritesheet_data):
    ...
    def __init_spritesheet(self, paths, spritesheet_data):
        ...
        if not (path.basename(paths["preloads"]) == ''):
            self.__preload_surfaces(paths["preloads"], spritesheet_data)

    def __preload_surfaces(self, preload_file, spritesheet_data):
        ...
        for node in preload_root:
            # render the image
            fn = node.attrib['name']
            spritesheet_data['preloads'][fn] = self.__create_surface(fn, spritesheet_data)
